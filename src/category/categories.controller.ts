...
export class CategoriesController {
	...
	@Post()
	async addCategory(@Body('title') catTitle: string) {
		const generatedId = await this.CategoriesService.insertCategory(catTitle);
	...
	async removeCategory(@Param('id') catId: string) {
		const result = await this.CategoriesService.deleteCategory(catId);
		return result;
	}
}
